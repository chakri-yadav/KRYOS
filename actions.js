...
function actionSyncLabel() {
  if (actionSyncState === 'local' && typeof syncState !== 'undefined' && syncState.status === 'connected' && syncState.userId) return 'Live sync ready';
  const labels = { local: 'Saved on this device', saving: 'Saving to cloud', synced: 'Updated across devices', error: 'Saved here - cloud unavailable' };
...
function renderActionVault() {
  ...
  actionsView.innerHTML = `<header class="action-title"><div><p class="section-kicker">KRYOS / ACTION VAULT</p><h1>The next right actions.</h1><p>Capture it, choose its state, and keep moving.</p><span id="action-sync-state" class="action-sync-state state-${actionSyncState}" role="status" aria-live="polite"><i></i>${escapeHtml(actionSyncLabel())}</span></div><div class="action-open-count"><strong>${summary.open}</strong><span>open commitments</span><small class="${summary.overdue ? 'has-overdue' : ''}">${attention}</small></div></header>
