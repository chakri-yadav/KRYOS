...
function launchEventsOn(date) { return launchStore().marketEvents.filter(item=>item.date===date); }
...
function launchMarketLabel(type){return ({application:'Application',connection:'Connection',message:'Direct message',followup:'Follow-up',visibility:'Profile / visibility'})[type]||type;}
...
function renderLaunchDialog(){
  ...
  const isMarket=launchDialog==='market',isPlatform=launchDialog==='platform';
  return `<dialog id="launch-dialog" class="money-dialog launch-dialog"><form id="launch-form"><header><div><p class="section-kicker">CAREER LAUNCH / ${isMarket?'MARKET':isPlatform?'PLATFORMS':'REHEARSE'}</p><h2>${isMarket?'Record real-world exposure':isPlatform?'Add a search source':'Record an interview repetition'}</h2></div><button type="button" data-launch-close aria-label="Close">×</button></header><div class="money-form-grid">${isMarket?market:isPlatform?platform:mock}</div><footer><button type="button" class="secondary-button" data-launch-close>Cancel</button><button class="primary-button">Save evidence</button></footer></form></dialog>`;
}
...
document.addEventListener('submit',event=>{if(event.target.id!=='launch-form')return;event.preventDefault();const f=new FormData(event.target),store=launchStore(),now=new Date().toISOString();if(f.get('type')==='market')store.marketEvents.push({id:createId(),date:f.get('date'),type:f.get('action'),company:String(f.get('company')||'').trim(),role:String(f.get('role')||'').trim(),person:String(f.get('person')||'').trim(),channel:f.get('channel'),stage:f.get('stage'),note:String(f.get('note')||'').trim(),createdAt:now});else if(f.get('type')==='platform'){const name=String(f.get('name')||'').trim();if(name&&!store.platforms.some(item=>item.name.toLowerCase()===name.toLowerCase()))store.platforms.push({id:`platform-${createId()}`,name});}else store.mockSessions.push({id:createId(),date:f.get('date'),mode:f.get('mode'),minutes:Number(f.get('minutes'))||0,focus:String(f.get('focus')||'').trim(),confidenceBefore:Number(f.get('confidenceBefore'))||null,confidenceAfter:Number(f.get('confidenceAfter'))||null,note:String(f.get('note')||'').trim(),createdAt:now});saveTasks();launchDialog='';renderLaunchView();});
