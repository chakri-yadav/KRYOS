...
const COVENANT = { start:'2026-09-19', end:'2026-11-12', requiredDays:55 };
...
function covenantStats(today=toDateKey()) {
  ...
  return {qualified,breaches,reviewApproved,unlocked:qualified>=COVENANT.requiredDays&&reviewApproved,remaining:Math.max(0,COVENANT.requiredDays-qualified),readyDate,start};
}
...
function renderJournalRewards() {
  ...
  rewardsView.innerHTML=`<header class="reward-hero"><div><p class="section-kicker">EFFORT / CONSISTENCY / ENJOYMENT</p><h1>Make room for earned enjoyment.</h1><p>Rest, meals, sleep, ordinary walks and supportive contact are always available.</p><span role="status">${escapeHtml(rewardCloudNotice||'Saved locally · cloud confirmation required for redemption')}</span></div><div><small>AVAILABLE CREDITS</small><strong>${stats.balance}</strong><span>${stats.earned} earned · ${stats.spent} reserved or spent</span></div></header>
    ...
    <section class="reward-review"><header><div><p class="section-kicker">ONE REVIEW ACROSS KRYOS</p><h2>${e.total}/10 supported on ${formatDateKey(date)}</h2><p>${e.workspaces.length} of ${REWARD_WORKSPACES.length} evidence workspaces contributed.</p></div><input id="reward-date" type="date" max="${toDateKey()}" value="${date}" aria-label="Review date"></header><div class="reward-gates"><div class="${gate.pointsMet?'met':''}"><i></i><span>Evidence threshold</span><strong>${gate.pointsMet?'Met':`${gate.remainingPoints} more needed`}</strong></div><div class="${gate.groundedMet?'met':''}"><i></i><span>Grounded action</span><strong>${gate.groundedMet?'Met':'Career, Launch or Responsibility'}</strong></div><div class="${gate.qualified?'met':''}"><i></i><span>Local credit result</span><strong>${gate.qualified?`+${assessmentCredits(e)} credits`:'Not qualified yet'}</strong></div></div><div class="reward-evidence">${Object.entries(REWARD_CATEGORIES).map(([key,category])=>`<details><summary><span>${category.title}</span><b>${e.scores[key]}/${category.cap}</b><progress max="${category.cap}" value="${e.scores[key]}"></progress></summary>${e.buckets[key].length?e.buckets[key].map(row=>`<p>${escapeHtml(row.title)}<small>${escapeHtml(row.source||'KRYOS')}</small></p>`).join(''):'<p>No qualifying evidence recorded.</p>'}</details>`).join('')}</div><footer><span>${review?`Reviewed under rules ${review.ruleVersion||1}`:'Awaiting review'} · credits save locally first</span><button class="primary-button" data-reward-review="${date}" ${review&&review.ruleVersion!==2?'disabled':''}>${review?'Recalculate review':'Confirm daily review'}</button></footer><p role="status">${escapeHtml(rewardNotice||rewardNoticeText(gate,review))}</p></section>
    <section class="reward-covenant"><div><p class="section-kicker">PROTECTED / ${COVENANT.requiredDays} REVIEWED DAYS</p><h2>${c.unlocked?'Astrology review approved':'Astrology remains protected'}</h2><p>${c.qualified}/${COVENANT.requiredDays} kept days · ${c.breaches} lapses recorded · ${c.remaining} days remaining · ends November 12</p><p>A lapse pauses accumulation. Unknown days remain unknown. Credits cannot bypass this boundary.</p></div><div class="reward-covenant-grid">${Array.from({length:COVENANT.requiredDays},(_,i)=>`<i class="${i<c.qualified?'kept':''}" title="${i+1} of ${COVENANT.requiredDays} reviewed days">${i+1}</i>`).join('')}</div></section>
