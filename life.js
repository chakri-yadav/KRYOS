...
const LIFE_RHYTHM_HABITS = new Set(['breakfast', 'lunch', 'dinner', 'protein', 'supplements', 'water', 'face-wash', 'moisturizer', 'serum', 'eye-cream', 'sunscreen', 'exercise', 'hair-care', 'groceries', 'nama-japa', 'gita', 'chalisa', 'aditya', 'meditation', 'pranayama']);
...
function lifeStore() {
  ...
  taskState.life.innerCommand ||= {
    covenant: { startDate: '2026-09-26', endDate: '2026-11-12', days: 48, title: '48-Day Devi Sadhana' },
    containmentDays: [],
  };
...
function normalizeImportedContainment(containment) {
  ...
  const allowed = new Set(['astrology', 'social', 'information', 'validation']);
...
function renderContainmentCovenant() {
  ...
  return `<section class="containment-command"><header><div><p class="section-kicker">48-DAY DEVI SADHANA</p><h2>${day < 1 ? `Begins ${formatDateKey(covenant.startDate)}` : day > covenant.days ? 'Sadhana complete' : `Day ${day} of ${covenant.days}`}</h2><p>${formatDateKey(covenant.startDate)} to ${formatDateKey(innerCommandEndDate())}</p></div><div class="containment-score"><strong>${stats.current}</strong><span>current aligned days</span><small>${stats.kept} aligned · ${stats.breaches} drift days recorded</small></div></header><div class="containment-body"><div class="containment-boundaries"><article><i>${typeof rhythmIcon === 'function' ? rhythmIcon('shield-check') : ''}</i><div><strong>No astrology seeking</strong><span>No readings, predictions, chart checking, or reassurance loops during the Sadhana.</span></div></article><article><i>${typeof rhythmIcon === 'function' ? rhythmIcon('smartphone-off') : ''}</i><div><strong>No unplanned Instagram scrolling</strong><span>Protect attention from feeds that keep meaningful work unfinished.</span></div></article><article><i>${typeof rhythmIcon === 'function' ? rhythmIcon('message-circle-off') : ''}</i><div><strong>Information follows action</strong><span>News, interviews, and analysis become bounded rewards after purposeful work.</span></div></article><article class="return"><i>${typeof rhythmIcon === 'function' ? rhythmIcon('rotate-ccw') : ''}</i><div><strong>Return without punishment</strong><span>Name the drift, complete one bounded recovery action, and resume the promise.</span></div></article></div><div class="containment-calendar" style="--covenant-progress:${Math.round(visibleDay / covenant.days * 100)}%"><div class="containment-track"><i></i></div><div class="containment-days">${stats.dates.map((date,index) => `<button class="${innerCommandStatus(date)} ${date === toDateKey() ? 'today' : ''}" type="button" title="Day ${index + 1} · ${formatDateKey(date)} · ${innerCommandStatus(date)}" ${date > toDateKey() ? 'disabled' : ''}><span>${index + 1}</span></button>`).join('')}</div><div class="containment-legend"><span><i class="kept"></i>Aligned</span><span><i class="breach"></i>Drift recorded</span><span><i></i>Unrecorded</span></div><div class="containment-today"><div><span>TODAY'S TRUTH</span><strong>${todayStatus === 'kept' ? 'Day aligned' : todayStatus === 'breach' ? 'Drift acknowledged. Return now.' : 'Not recorded yet'}</strong></div><button class="primary-button" data-containment="kept" ${day < 1 || day > covenant.days ? 'disabled' : ''}>Aligned today</button><button class="secondary-button" data-containment="breach" ${day < 1 || day > covenant.days ? 'disabled' : ''}>Record drift</button></div></div></div></section>`;
}
function renderContainmentDialog() {
  if (!containmentDialogOpen) return '';
  return `<dialog id="containment-dialog" class="money-dialog containment-dialog"><form id="life-containment-form"><header><div><p class="section-kicker">TRUTH, THEN RETURN</p><h2>What pulled attention away?</h2></div><button type="button" data-containment="close" aria-label="Close">×</button></header><div class="containment-breach-options"><label><input type="radio" name="boundary" value="astrology" required><span><strong>Astrology seeking</strong><small>Reading, prediction, chart check, or reassurance loop</small></span></label><label><input type="radio" name="boundary" value="social" required><span><strong>Instagram scrolling</strong><small>Unplanned feed or reel use</small></span></label><label><input type="radio" name="boundary" value="information" required><span><strong>Information replaced action</strong><small>News, interviews, or analysis used instead of the needed work</small></span></label><label><input type="radio" name="boundary" value="validation" required><span><strong>Validation-seeking contact</strong><small>Contact used primarily to escape or obtain reassurance</small></span></label><label class="containment-note"><span><strong>What happened?</strong><small>Brief facts, without punishment or justification.</small></span><textarea name="note" maxlength="500"></textarea></label></div><footer><button type="button" class="secondary-button" data-containment="close">Cancel</button><button class="primary-button">Record and return</button></footer></form></dialog>`;
}
